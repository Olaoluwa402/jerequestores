...
router.get("/", async (req, res) => {
  ...
  const blogs = await Blog.find().sort({createdAt: 'desc'}).limit(2);
              ...
              .then(user => {
                    if(req.session.oldUrl){
                       const oldUrl = req.session.oldUrl;
                        ...
                        req.flash("success", "You are now registered and can log in " + user.lastName);
...
router.get("/admin/settings", isLoggedIn, async (req, res) => {
  ...
  try{
        const productLimit = await ProductLimit.findOne({});
       res.render("admin/settings", {productLimit: productLimit});
  } catch(e){
      console.log(e);
  }
